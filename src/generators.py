    ...
    *shuffle: flag to shuffle IDs after each epoch.
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            # Data augmentation, random rotation of input and output
        ...
        # Normalize dose, ray and intensity values separately
...
class DataGenerator_inference(Sequence):
    """
    Generates data for Keras.
    *path: path to the data.
    *list_IDs: a list with the file identifiers.
    ...
    *scale: dict with max and min values
    ...
    def __init__(self, path, list_IDs, batch_size, scale):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # Load batch of data
        for i, ID in enumerate(list_IDs_temp):
            X[i,] = np.load(self.path + str(ID) + '.npz')['vol']
            r[i,] = np.load(self.path + str(ID) + '.npz')['ray']

        # Normalize dose, ray tracing and intensity values separately
        X = (X - self.x_min) / (self.x_max - self.x_min)
        r = (r - self.r_min) / (self.r_max - self.r_min)

        return [np.expand_dims(X, -1), np.expand_dims(r, -1)]