...
def rotations_to_target_range(angle, direction="counterclockwise"):
    """ Calculates the number of rotations needed to rotate a target angle """
    if not (0 <= angle < 360):
        raise ValueError("The angle must be between 0 and 360 degrees.")

    if direction not in ["clockwise", "counterclockwise"]:
        raise ValueError("The direction must be 'clockwise' or 'counterclockwise'.")

    target_range = range(225, 316)
    rotations = 0

    while angle not in target_range:
...
def load_model_and_scale(name):
    """ Load the weight and the scaling parameters """
    with open("/home/moderato/Documents/iDoTa/hyperparam.json", "r") as hfile:
        param = json.load(hfile)

    checkpoints_dir = '/home/moderato/Documents/iDoTa/checkpoints/'
...
def time_analysis(model, testIDs, path, scale, ikey='vol', okey='dose',
    ...
    input_dim = tuple(np.load(path+'0.npz')[ikey].shape)
